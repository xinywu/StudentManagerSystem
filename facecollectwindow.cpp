...
#include "ui_facecollectwindow.h"
#include "QDir"
#include "QFileDialog"
#include "QDebug"
...
/////////////////////////////////////
FaceCollectWindow::FaceCollectWindow(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::FaceCollectWindow)
{
    m_currenttab = -1;
    ui->setupUi(this);
    ...
    // 为 SeetaFace 使用独立数据库连接
    const QString faceConnectionName = QStringLiteral("seetaface_connection");

    if (QSqlDatabase::contains(faceConnectionName)) {
        m_database = QSqlDatabase::database(faceConnectionName);
    } else {
        m_database = QSqlDatabase::addDatabase("QSQLITE", faceConnectionName);
    }
    ...
    if(!bconfigfind)
    {
        ...
        qDebug() << sql;
        // 使用 SeetaFace 人脸数据库执行语句
QSqlQuery q(sql, m_database);
        if(!q.exec())
        {
...
FaceCollectWindow::~FaceCollectWindow()
{
...
void FaceCollectWindow::cleardata()
{
...
void FaceCollectWindow::getdatas()
{
      int i = 0;
      // 从 SeetaFace 独立数据库读取人脸数据
QSqlQuery q("select * from " + m_table + " order by id asc", m_database);
      while(q.next())
      {
...
void FaceCollectWindow::editrecord()
{
    //QPushButton *button = (QPushButton *)sender();
    //qDebug() << button->property("id").toInt() << ", edit";
}

void FaceCollectWindow::deleterecord()
{
    QPushButton *button = (QPushButton *)sender();
    ...
    // 从 SeetaFace 人脸数据库删除记录
QSqlQuery query(
    "delete from " + m_table + " where id=" + QString::number(id),
    m_database
);
...
void FaceCollectWindow::showfaceinfo()
{
...
void FaceCollectWindow::onrecognize(int pid, const QString & name, const QString & imagepath, float score, const QImage &image, const QRect &rc)
{
    ...
    */
    ...
    // 获取学生管理系统的默认数据库连接
    QSqlDatabase studentDatabase = QSqlDatabase::database();

    if (!studentDatabase.isOpen()) {
        qWarning() << "学生数据库未打开";
        return;
    }

    // 根据人脸库姓名查询学生信息
    QSqlQuery studentQuery(studentDatabase);
    ...
    studentQuery.bindValue(":name", name);
    ...
    if (!studentQuery.next()) {
        qWarning() << "学生表中没有找到姓名：" << name;
        return;
    }

    const QString studentNo = studentQuery.value(0).toString();
    const QString studentName = studentQuery.value(1).toString();
    const QDateTime currentTime = QDateTime::currentDateTime();

    // 将识别结果写入考勤记录表
    QSqlQuery attendanceQuery(studentDatabase);
    attendanceQuery.prepare(
        "INSERT OR IGNORE INTO attendance_records ("
        "student_no, student_name, attendance_date, "
    ...
    attendanceQuery.bindValue(":student_no", studentNo);
    attendanceQuery.bindValue(":student_name", studentName);
    attendanceQuery.bindValue(
        ":attendance_date",
    ...
    attendanceQuery.bindValue(":status", QStringLiteral("正常"));
    attendanceQuery.bindValue(":similarity", score);

    if (!attendanceQuery.exec()) {
        qWarning() << "写入考勤记录失败："
                   << attendanceQuery.lastError().text();
        return;
    }

    // 输出本次签到结果
    if (attendanceQuery.numRowsAffected() > 0) {
        qDebug() << "签到成功：" << studentNo << studentName << score;
    }

}

void FaceCollectWindow::onupdateui(const QImage & image)
{
    int a = ui->previewlabel->width();
    int b = ui->previewlabel->height();
    QImage ime = image.scaled(a,b);
...
void FaceCollectWindow::onvideothreadend(int value)
{
...
void FaceCollectWindow::on_dbsavebtn_clicked()
{
...
void FaceCollectWindow::on_previewrunbtn_clicked()
{
...
void FaceCollectWindow::on_previewstopbtn_clicked()
{
    m_videothread->m_exited = true;
}

void FaceCollectWindow::on_settingsavebtn_clicked()
{
    /*
...
void FaceCollectWindow::on_rotatebtn_clicked()
{
   QTransform matrix;
    matrix.rotate(90);
...
void FaceCollectWindow::on_tabWidget_currentChanged(int index)
{
...
void FaceCollectWindow::on_addimagebtn_clicked()
{
...
void FaceCollectWindow::on_menufacedbbtn_clicked()
{
...
void FaceCollectWindow::on_menusettingbtn_clicked()
{

     ui->tabWidget->setCurrentIndex(3);
}

void FaceCollectWindow::on_previewclearbtn_clicked()
{
    ui->previewtableview->setUpdatesEnabled(false);
...
void FaceCollectWindow::on_menuopenvideofile_clicked()
{
...
void FaceCollectWindow::on_menuopenpicturefile_clicked()
{
    QString fileName = QFileDialog::getOpenFileName(this, tr("open image file"),
                                                         "./" ,
                                                        "JPEG Files(*.jpg *.jpeg);;PNG Files(*.png);;BMP Files(*.bmp)");
...
void FaceCollectWindow::on_menuopencamera_clicked()
{
    m_type.type = 0;
    m_type.filename = "";
    m_type.title = "Open Camera: 0";
...
void FaceCollectWindow::on_addfilesbtn_clicked()
{
...
void FaceCollectWindow::oninputfilesupdateui(std::vector<DataInfo *> * datas)
{
    DataInfo * info = NULL;
...
void FaceCollectWindow::on_settingselectmodelbtn_clicked()
{
...
void FaceCollectWindow::closeEvent(QCloseEvent *event)
{
    m_videothread->m_exited = true;
    m_videothread->wait();
